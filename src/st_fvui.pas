...
begin
  ...
  begin
    ...
    begin
      ...
      PaneTerm[i] := -2;
      {$IFDEF WINDOWS}
      // A plain shell is the portable recovery path. The failed title keeps
      // the reason visible without feeding POSIX printf/exec to cmd.exe.
      FallbackCmd := '';
      {$ELSE}
      FallbackCmd := 'printf ' +
      ...
      Scr[i] := TScreen.Create(pw, ph, AMaxSB);
      Panes[i] := TPty.Create;
      SpawnOK := Panes[i].Spawn(Cfg.Shell, OsUserHome,
