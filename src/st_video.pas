...
uses
  ...
  // at all and its .text is identical to one built without this unit. The
  // separate `make perf` binary carries it. Same reasoning as debug-heap.
...
begin
  ...
  {$ENDIF}
  // Handing the frame to the output reactor replaces the earlier attempt to
  // drop a frame when the terminal was not draining. That guard was measured
  // and did not work: select reports a pipe writable while ANY space is free,
  // so with an ~11 KiB buffer and 60 KiB frames it never fired once while the
  // client still blocked inside write(). A queue the UI thread never waits on
