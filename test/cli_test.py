...
for version_args in (['version'], ['--version'], ['-V']):
    ...
    check(' '.join(version_args) + ' reports VERSION exactly',
          ...
          r.stderr == '')
# The binary reads VERSION through a generated include, so it can never drift.
# These cannot: superterm_version.rh is only regenerated by a Windows build,
# the Inno Setup script carries its own literal, and the README is prose. A
# Windows Details tab reporting one release while the binary reports another
# is exactly what the generated header was introduced to prevent, so the
# agreement is asserted here rather than trusted.
def file_text(*parts):
    with open(os.path.join(ROOT, *parts), encoding='utf-8') as handle:
        return handle.read()


VER_RH = file_text('src', 'superterm_version.rh')
MAJOR, MINOR, PATCH = EXPECTED_VERSION.split('.')
check('the Windows version resource matches VERSION',
      '#define ST_VER_STRING "' + EXPECTED_VERSION + '"' in VER_RH and
      '#define ST_VER_MAJOR ' + MAJOR in VER_RH and
      '#define ST_VER_MINOR ' + MINOR in VER_RH and
      '#define ST_VER_PATCH ' + PATCH in VER_RH)
check('the Windows installer matches VERSION',
      '#define AppVersion "' + EXPECTED_VERSION + '"' in
      file_text('packaging', 'windows', 'superterm.iss'))
README = file_text('README.md')
check('the README title and download link match VERSION',
      README.startswith('# superterm ' + EXPECTED_VERSION + '\n') and
      '**[Download ' + EXPECTED_VERSION + ']' in README)
check('the changelog opens with VERSION',
      file_text('CHANGELOG.md').split('\n')[2].startswith(
          '## ' + EXPECTED_VERSION + ' '))
