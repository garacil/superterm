...
CONST
   ...
   MaxViewWidth = 8192;                                { Max view width }
    ...
    if (p=staticVar2.target) then
     begin
       ...
       if (G^.buffer<>Nil) then
        begin
           { G^.buffer is the global video buffer, not a group-local buffer. }
           BufPos:=ScreenWidth * staticVar2.y + x1;
