...
procedure SetMessageBoxLanguage(ASpanish: Boolean);
...
var
  ...
  MessageBoxSpanish: Boolean = False;
...
BEGIN
   ...
   With Dlg^ Do Begin
     ...
     Control := New(PStaticText, Init(R, S));         { Create static text }
      ...
      For I := 0 To High(ButtonList) Do
        ButtonList[I] := Nil;                           { Clear button list }
      For I := 0 To 3 Do
   ...
   With Dialog^ Do Begin
     ...
     R.Assign(Size.X - 24, Size.Y - 4, Size.X - 14,
      ...
      If MessageBoxSpanish Then
        Insert(New(PButton, Init(R, 'A~c~eptar', cmOk,
          bfDefault)))
      Else
     ...
     Inc(R.B.X, 12);                                  { New end x position }
      If MessageBoxSpanish Then
        Insert(New(PButton, Init(R, 'Cancelar', cmCancel,
          bfNormal)))
      Else
...
begin
  ...
  begin
    ...
    MsgBoxTitles[1] := 'Error';
    MsgBoxTitles[2] := 'Informacion';
    MsgBoxTitles[3] := 'Confirmacion';
  end
  else
  begin
    ButtonName[0] := slYes;
