...
```sh
./configure && make          # release binary in bin/
make test                    # the full suite
sudo make install            # /usr/local by default
```

Build modes (`debug`, `debug-heap`, `perf`), dependencies, the configure
options and the install layout are in **[docs/BUILDING.md](docs/BUILDING.md)**.

## Command-line control

Every interactive action has a scriptable equivalent, in English and Spanish:

```sh
superterm sessions                       # list
superterm send work:1 'make -j8'         # type into a pane
superterm capture work:1                 # read a pane back
```

The complete command reference, exit codes and multi-user examples are in
**[docs/CLI.md](docs/CLI.md)**; `superterm help <command>` prints the same
contextually.
...
`superterm` with no configuration opens a wizard that builds the first
workspace: shell, layout, window class and profile. See
**[docs/WIZARD.md](docs/WIZARD.md)**.

## Configuration

Everything lives under `~/.superterm/`: `superterm.ini` for user settings,
plus window classes, profiles and the fallback session. The file format, every
section and key, the desktop background and the SSH routing are documented in
